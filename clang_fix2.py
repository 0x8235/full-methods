...
def get_options():
    mc_path_default = str(Path.home() / "trustfi")
    ...
    parser.add_argument("-m", "--trustfi", metavar="DIR", default=mc_path_default,
                        help="TrustFi path prefix (default: %(default)s)")
    ...
    if not Path(options.trustfi).exists():
        parser.error("{!r}: TrustFi path does not exist".format(options.trustfi))
    ...
    logger.info("  TrustFi: {!r}".format(options.trustfi))
...
def main():
    ...
    os.chdir(str(Path(options.trustfi) / "v8build" / "v8"))
    for spec in specs:
